...
class _DragMarkersState extends State<DragMarkers> {
  ...
  Widget build(BuildContext context) {
    ...
    for (var c = 0; c < widget.markers.length; c++) {
      ...
      dragMarkers.add(DragMarkerWidget(key: widget.markers[c].key ?? ValueKey(c), mapState: mapState, marker: widget.markers[c]));
    }
  ...
  static bool _boundsContainsMarker(FlutterMapState? map, DragMarker marker) {
    ...
    final sw = CustomPoint(pxPoint.x + leftPortion - 100, pxPoint.y - bottomPortion + 100);
...
class DragMarkerWidget extends StatefulWidget {
  const DragMarkerWidget({Key? key, this.mapState, required this.marker, AnchorPos? anchorPos})
...
class _DragMarkerWidgetState extends State<DragMarkerWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    Widget displayMarker = feedBackEnabled ? marker.feedbackBuilder!(context) : marker.builder!(context);
    ...
    return GestureDetector(
      ...
      child: Stack(children: [
        Positioned(
            ...
            left: pixelPosition.x + ((isDragging) ? marker.feedbackOffset.dx : marker.offset.dx),
            top: pixelPosition.y + ((isDragging) ? marker.feedbackOffset.dy : marker.offset.dy),
            child: widget.marker.rotateMarker
                ? Transform.rotate(angle: -widget.mapState!.rotationRad, child: displayMarker)
                : displayMarker)
      ]),
    );
  }
  ...
  void _pan(Offset localPosition) {
    ...
    if (marker.updateMapNearEdge) {
      ...
      if (pixelB != null && pixelPoint != null) {
        if (pixelPoint.x + marker.width * marker.nearEdgeRatio >= pixelB.topRight.x) autoOffsetX = marker.nearEdgeSpeed;
        if (pixelPoint.x - marker.width * marker.nearEdgeRatio <= pixelB.bottomLeft.x) autoOffsetX = -marker.nearEdgeSpeed;
        if (pixelPoint.y - marker.height * marker.nearEdgeRatio <= pixelB.topRight.y) autoOffsetY = -marker.nearEdgeSpeed;
        if (pixelPoint.y + marker.height * marker.nearEdgeRatio >= pixelB.bottomLeft.y) autoOffsetY = marker.nearEdgeSpeed;
      }
      ...
      if ((autoOffsetY != 0.0) || (autoOffsetX != 0.0)) {
        ...
        if ((autoDragTimer == null || autoDragTimer?.isActive == false) && (isDragging == true)) {
          autoDragTimer = Timer.periodic(const Duration(milliseconds: 10), (Timer t) {
    ...
    setState(() {
      markerPoint = LatLng(markerPointStart.latitude + deltaLat, markerPointStart.longitude + deltaLon);
  ...
  void adjustMapToMarker(DragMarkerWidget widget, double autoOffsetX, double autoOffsetY) {
    ...
    if (oldMapPos != null) {
      newMapLatLng = mapState?.unproject(CustomPoint(oldMapPos.x + autoOffsetX, oldMapPos.y + autoOffsetY));
      oldMarkerPoint = mapState?.project(markerPoint);
    }
    if (mapState != null && newMapLatLng != null && oldMarkerPoint != null) {
      markerPoint = mapState.unproject(CustomPoint(oldMarkerPoint.x + autoOffsetX, oldMarkerPoint.y + autoOffsetY));
  ...
  void onLongPanEnd(LongPressEndDetails details) {
    _end();
